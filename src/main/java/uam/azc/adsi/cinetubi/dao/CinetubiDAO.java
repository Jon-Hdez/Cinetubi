...
public class CinetubiDAO {
    ...
    public static ArrayList<Asiento> obtenerAsientos(int id_sala) {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
             ...
             ResultSet rs = stmt.executeQuery("SELECT * FROM asiento WHERE id_sala="+ id_sala +";")) {
    ...
    public static ArrayList<Funcion> obtenerFunciones(int idFuncion){
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT * FROM funcion"+ idFuncion +";")) {

            while (rs.next()) {
                int idSala = rs.getInt("id_Sala");
                int numero = rs.getInt("numero");
                Funcion funcion = new Funcion(idSala, numero);
    ...
    public static ArrayList<Sala> obtenerSalas(){
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT * FROM sala where id in (select sala FROM funcion);")) {
